...
1. **가로 모드 권장(태블릿)**
   - 태블릿은 **가로 모드**가 가장 보기 좋습니다. 설정에서 **화면 회전 잠금(가로)** 을 켜세요.
   - 단, 세로로 들어도 **자동으로 세로 레이아웃**으로 전환되어 그대로 플레이됩니다(아래 6-1 참고).
2. **전체 화면(키오스크) 실행**
...
## 6-1. 스마트폰(모바일)에서 사용하기

같은 주소(URL)를 **스마트폰**으로 열면 별도 설치 없이 **세로 화면 전용 레이아웃**으로 자동 전환됩니다. (별도 모바일 파일 없이 하나의 코드가 반응형으로 대응)

### 자동으로 바뀌는 것
- **화면 방향/크기 자동 감지**: 가로↔세로 회전 시 레이아웃이 즉시 재배치됩니다(CSS 미디어쿼리 + JS 뷰포트 감지 병행).
- **세로 레이아웃**: 아기 캐릭터는 **중앙 상단**, 제품 이미지는 **하단에 크게** 배치되어 아래에서 위로 문지르기 편합니다.
- **겹침 방지**: 작은 화면에서 HUD(남은 계면이·게이지·타이머)와 버튼이 겹치지 않도록 압축·재배치됩니다.
- **난이도 자동 완화**: 작은 화면(스마트폰)에서는 제한 시간이 늘고(60→80초), 계면이 수·체력·도망 속도가 줄어 아이도 쉽게 클리어할 수 있습니다.
- **터치 우선**: 모바일은 **「터치로 플레이하기」** 버튼이 위에 강조되어 가장 안정적으로 동작합니다.

### 손 인식(카메라)은?
- 모바일 브라우저(**iPhone Safari / Android Chrome**)에서도 **가능하면 손 인식이 동작**합니다.
- 권한 거부·미지원·오류 시 안내가 뜨고 **자동으로 터치 모드로 진행**할 수 있습니다(행사 중 멈춤 없음).
- 카메라 미리보기는 우상단에 작게 표시되며, 좌상단 **접기 버튼(▾)** 으로 작게 접거나 펼 수 있습니다. 미리보기는 터치를 가로채지 않아 플레이에 방해되지 않습니다.

### 권장 사항
- **세로 모드 권장**(스마트폰). 가로로 돌려도 동작합니다.
- iPhone Safari / Android Chrome: 공유 → **홈 화면에 추가** 하면 풀스크린 앱처럼 실행됩니다.
- 손 인식을 쓰려면 **https(또는 localhost)** 환경이어야 합니다 → GitHub Pages/Netlify 배포본은 https라 그대로 사용 가능합니다.
- 지원 환경: **iPhone Safari, Android Chrome, iPad Safari** 모두 대응.
